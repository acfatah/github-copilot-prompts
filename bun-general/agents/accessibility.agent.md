...
bunx --bun @axe-core/cli http://localhost:3000 --exit

# Crawl with pa11y and generate HTML report
bunx --bun pa11y http://localhost:3000 --reporter html > a11y-report.html

# Lighthouse CI (accessibility category)
bunx --bun lhci autorun --only-categories=accessibility
...
jobs:
  axe-pa11y:
    ...
    steps:
      ...
      - run: bun install
      - run: bun run build
      - run: bunx serve -s dist -l 3000 &
      - run: bunx wait-on http://localhost:3000
      - run: bunx @axe-core/cli http://localhost:3000 --exit
        continue-on-error: false
      - run: bunx pa11y http://localhost:3000 --reporter ci
```
